...
exports.getFunctions = (html5playerfile, options) =>
    exports.cache.getOrSet(html5playerfile, async () => {
        const body = await utils
            .exposedMiniget(html5playerfile, options)
            .text();
...
exports.extractFunctions = (body) => {
    ...
    const extractDecipher = () => {
        ...
        if (functionName && functionName.length) {
            ...
            if (ndx >= 0) {
                ...
                let functionBody = `var ${functionStart}${utils.cutAfterJS(
                    subBody
                )}`;
    ...
    const extractNCode = () => {
        ...
        console.log("NCode function name", functionName);
        if (functionName && functionName.length) {
            const functionStart = `${functionName}=function(a)`;
            const ndx = body.indexOf(functionStart);
            if (ndx >= 0) {
